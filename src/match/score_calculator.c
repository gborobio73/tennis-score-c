...
static void new_set(Score* new_score){
	int who_won = new_score->who_won_the_point;
...
static void new_game (Score* new_score){
	int who_won = new_score->who_won_the_point;
    ...
    { 
    	new_set(new_score); /* it was either tiebreak or 6 to 5, or less */
    }
    else if(new_score->games[who_won]  == 5)
    {
    	if(new_score->games[who_lost] ==6)
    	{
 		...
 		{
        	new_set(new_score); /* end of set: 6 to 4 or less */
      	}
      	else
      	{
        	new_score->games[who_won] ++;
...
static bool tiebreak_should_change_serve(Score* new_score){
	int who_won = new_score->who_won_the_point; 
	int who_lost = !who_won;
...
static void calculate_tiebreak_new_score(Score* new_score ){
	int who_won = new_score->who_won_the_point; 
	int who_lost = !who_won;
	...
	{			
		...
		if (tiebreak_should_change_serve(new_score)) 
		{ 
			new_score->who_serves = !(new_score->who_serves);
		}
	}
	else
	{
		...
		{
			new_game(new_score); /* tiebreak end*/
		}
		else
		{
			new_score->tie_break_points[who_won]++;		
...
void score_calculator_calculate_new_from_to(Score* current_score, int who_won, Score* new_score){
    ...
    new_score->who_won_the_point = who_won;

	int who_lost = !who_won;
    
    if(new_score->is_tie_break)
    {
    	calculate_tiebreak_new_score(new_score);
    }
    else
    {
   		if(strcmp(new_score->points[who_won], ad) == 0)
   		{
			new_game(new_score); /* opponent won game, had AD */			
		}
		...
		{			
			...
			{
			  new_game(new_score); /* who lost had less than 40 */
			}      
		}
